...
def analyze_data_function(data, parameters):
    ...
    num_muons_event = kernels.sum_in_offsets(
        backend,
        muons.offsets,
    ...
    leading_muon_pt = kernels.get_in_offsets(
        backend,
        muons.offsets,
        muons.pt,
        inds,
    ...
    hist_muons_pt = Histogram(
        *kernels.histogram_from_vector(
            backend,
            leading_muon_pt[mask_events_dimuon],
            weights[mask_events_dimuon],
            bins,
        )
    )
...
if __name__ == "__main__":
    ...
    results = dataset.analyze(
        analyze_data_function, verbose=True, parameters={"muons_ptcut": 30.0}
    )
