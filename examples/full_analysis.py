...
class DNNModel:
    ...
    def eval(self, X, use_cuda):
        ...
        rets = [
            self.NUMPY_LIB.array(m.predict(X, batch_size=10000)[:, 0])
...
def get_selected_electrons(
    electrons, pt_cut_leading, pt_cut_subleading, aeta_cut, iso_cut
):
    this_worker = get_worker_wrapper()
    NUMPY_LIB = this_worker.NUMPY_LIB
    backend = this_worker.backend
...
def apply_lepton_corrections(leptons, mask_leptons, lepton_weights):
    ...
    ha_kernels.get_bin_contents(
        backend, leptons.pt, lepton_weights[:, 0], lepton_weights[:-1, 1], corrs
    )
    ...
    corr_per_event = ha_kernels.prod_in_offsets(
        backend, leptons.offsets, corrs, all_events, mask_leptons
    )
...
def fill_histograms_several(
    hists, systematic_name, histname_prefix, variables, mask, weights, use_cuda
):
    this_worker = get_worker_wrapper()
    NUMPY_LIB = this_worker.NUMPY_LIB
    backend = this_worker.backend
    ...
    for array, varname, bins in variables:
        if (
            len(array) != len(variables[0][0])
            or len(array) != len(mask)
    ...
    for weight_name, weight_array in weights.items():
        if use_cuda:
            nblocks = 32
            out_w = NUMPY_LIB.zeros(
                (len(variables), nblocks, max_bins), dtype=NUMPY_LIB.float32
            )
            out_w2 = NUMPY_LIB.zeros(
                (len(variables), nblocks, max_bins), dtype=NUMPY_LIB.float32
            )
            ...
            out_w2 = NUMPY_LIB.zeros(
                (len(variables), max_bins), dtype=NUMPY_LIB.float32
            )
        ...
        for ihist in range(num_histograms):
            ...
            target_histogram = Histogram(
                out_w_separated[ihist], out_w2_separated[ihist], bins
            )
...
def run_analysis(dataset, out, dnnmodel, use_cuda, ismc):
    ...
    for ijec, sdir, jec in all_jecs:
        ...
        if use_cuda:
            this_worker.kernels.max_val_comb[32, 1024](
                jets.btag, jets.offsets, best_comb_3j, best_btag
            )
            cuda.synchronize()
        else:
            this_worker.kernels.max_val_comb(
                jets.btag, jets.offsets, best_comb_3j, best_btag
            )
        ...
        dijet_inv_mass, dijet_pt = compute_inv_mass(
            jets, selected_events, sel_jet & first_two_jets, use_cuda
        )

        sumpt_jets = ha_kernels.sum_in_offsets(
            backend, jets.offsets, jets.pt, selected_events, sel_jet
        )
    ...
    print(
        "run_analysis: {0:.2E} events in {1:.2f} seconds, speed {2:.2E} Hz".format(
            dataset.numevents(), t1 - t0, speed
        )
    )
    return res
...
def load_dataset(
    datapath, filenames, ismc, nthreads, do_skim, NUMPY_LIB, ha, entrystart, entrystop
):
    ...
    print(
        "Loaded dataset, {0:.2f} MB, {1} files, {2} events".format(
            ds.memsize() / 1024 / 1024, len(ds.filenames), ds.numevents()
        )
    )
    ...
    if do_skim:
        ...
        ds.compact(masks)
        print(
            "Applied trigger bit selection skim, {0:.2f} MB, {1} files, {2} events".format(
                ds.memsize() / 1024 / 1024, len(ds.filenames), ds.numevents()
            )
        )

    # Compute the average length of the data structures
    avg_vec_length = np.mean(
        np.array(
            [[v["pt"].shape[0] for v in ds.structs[ss]] for ss in ds.structs.keys()]
        )
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--skim", action="store_true", help="Specify if skim should be done"
    )
    parser.add_argument(
        "--nthreads",
        action="store",
    ...
    parser.add_argument(
        "--out", action="store", help="Output file name", default="out.pkl"
    )
    parser.add_argument(
        "--njobs",
        action="store",
        help="Number of multiprocessing jobs",
        default=1,
        type=int,
    )
    parser.add_argument(
        "--njec", action="store", help="Number of JEC scenarios", default=1, type=int
    )
...
def load_and_analyze(args_tuple):
    ...
    t0 = time.time()
    ret = run_analysis(
        ds, "{0}_{1}".format(dataset, ichunk), this_worker.dnnmodel, args.use_cuda, ismc
    )
    t1 = time.time()
...
if __name__ == "__main__":
    ...
    for i in range(2):
        download_if_not_exists(
            ...
            "https://jpata.web.cern.ch/jpata/hepaccelerate/model_kf{0}.h5".format(i),
        )
    ...
    for dataset, fn_pattern, ismc in datasets:
        ...
        if len(filenames) == 0:
            raise Exception(
                "Could not find any filenames for dataset={0}: {{datapath}}/{{fn_pattern}}={1}/{2}".format(
                    dataset, args.datapath, fn_pattern
                )
            )
        ichunk = 0
        for fn in filenames:
    ...
    print(
        "Processed {0} events in {1:.1f} seconds, {2:.2E} Hz".format(
