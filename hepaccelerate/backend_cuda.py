...
try:
    ...
    print(
        "Could not import cupy or numba.cuda, hepaccelerate.backend_cuda not usable",
        file=sys.stderr,
    )
...
def spherical_to_cartesian(pt, eta, phi, mass):
    ...
    spherical_to_cartesian_kernel[32, 1024](
        pt, eta, phi, mass, out_px, out_py, out_pz, out_e
    )
    cuda.synchronize()
...
def cartesian_to_spherical(px, py, pz, e):
    ...
    cartesian_to_spherical_kernel[32, 1024](
        px, py, pz, e, out_pt, out_eta, out_phi, out_mass
    )
    cuda.synchronize()
            ...
            for ivec in range(ndatavec):
                bin_idx = np.int32(
                    searchsorted_devfunc_right(
                        bins[nbins_sum[ivec] : nbins_sum[ivec + 1]], data[ivec, iev]
                    )
                    - 1
                )
...
@cuda.jit
def select_opposite_sign_cudakernel(
    charges_offsets, charges_content, content_mask_in, content_mask_out
):
    xi = cuda.grid(1)
    xstride = cuda.gridsize(1)
...
@cuda.jit
def set_in_offsets_cudakernel(
    offsets, content, indices, target, mask_rows, mask_content
):
    xi = cuda.grid(1)
    xstride = cuda.gridsize(1)
...
def sum_in_offsets(offsets, content, mask_rows=None, mask_content=None, dtype=None):
    ...
    sum_in_offsets_cudakernel[32, 1024](
        offsets, content, mask_rows, mask_content, sum_offsets
    )
    cuda.synchronize()
    return sum_offsets
...
def max_in_offsets(offsets, content, mask_rows=None, mask_content=None):
    ...
    mask_rows, mask_content = make_masks(offsets, content, mask_rows, mask_content)
    max_in_offsets_cudakernel[32, 1024](
        offsets, content, mask_rows, mask_content, max_offsets
    )
    cuda.synchronize()
    return max_offsets
...
def min_in_offsets(offsets, content, mask_rows=None, mask_content=None):
    max_offsets = cupy.zeros(len(offsets) - 1, dtype=content.dtype)
    mask_rows, mask_content = make_masks(offsets, content, mask_rows, mask_content)
    min_in_offsets_cudakernel[32, 1024](
        offsets, content, mask_rows, mask_content, max_offsets
    )
    cuda.synchronize()
    return max_offsets
...
def get_in_offsets(offsets, content, indices, mask_rows=None, mask_content=None):
    ...
    mask_rows, mask_content = make_masks(offsets, content, mask_rows, mask_content)
    get_in_offsets_cudakernel[32, 1024](
        offsets, content, indices, mask_rows, mask_content, out
    )
    cuda.synchronize()
    return out
...
def set_in_offsets(
    offsets, content, indices, target, mask_rows=None, mask_content=None
):
    assert content.shape == mask_content.shape
    assert offsets.shape[0] - 1 == indices.shape[0]
    ...
    assert offsets.shape[0] - 1 == mask_rows.shape[0]
    mask_rows, mask_content = make_masks(offsets, content, mask_rows, mask_content)
    set_in_offsets_cudakernel[32, 1024](
        offsets, content, indices, target, mask_rows, mask_content
    )
    cuda.synchronize()
...
def histogram_from_vector(data, weights, bins, mask=None):
    ...
    if len(data) > 0:
        if mask is None:
            ...
            fill_histogram_masked[nblocks, nthreads](
                data, weights, bins, mask, out_w, out_w2
            )
...
def histogram_from_vector_several(variables, weights, mask):
    ...
    fill_histogram_several[nblocks, 256](
        stacked_array, weights, mask, stacked_bins, nbins, nbins_sum, out_w, out_w2
    )
    out_w = out_w.sum(axis=1)
    out_w2 = out_w2.sum(axis=1)
    out_w_separated = [
        cupy.asnumpy(out_w[i, 0 : nbins[i] - 1]) for i in range(num_histograms)
    ]
    out_w2_separated = [
        cupy.asnumpy(out_w2[i, 0 : nbins[i] - 1]) for i in range(num_histograms)
    ]
    ...
    for ibin in range(len(all_bins)):
        ret += [
            (
                out_w_separated[ibin],
                out_w2_separated[ibin],
                cupy.asnumpy(all_bins[ibin]),
            )
        ]
    return ret
...
def compute_new_offsets(offsets_old, mask_objects, offsets_new):
    ...
    compute_new_offsets_cudakernel[32, 1024](
        offsets_old, mask_objects, counts, offsets_new
    )
    cuda.synchronize()
